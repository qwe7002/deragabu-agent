...
fn capture_cursor() -> Result<Option<CursorEvent>> {
    unsafe {
        ...
        let cached = capture_full_cursor(hcursor)?;
...
unsafe fn capture_full_cursor(hcursor: HCURSOR) -> Result<CachedCursor> {
    ...
    if is_monochrome {
        // Monochrome cursor - never animated, use mask-based capture
    ...
    // Color cursor - probe animation frames using the original HCURSOR handle.
    // Win32: HCURSOR and HICON are the same underlying HANDLE type, safe to transmute.
    let hicon_raw: HICON = mem::transmute(hcursor);
    let frames = probe_animation_frames(hicon_raw, width, height)?;

    if frames.len() <= 1 {
        // Static cursor
        let rgba = if frames.is_empty() {
            render_cursor_frame(hicon_raw, width, height, 0)?
        } else {
            frames[0].clone()
        };
...
/// Probe all unique animation frames for a cursor by iterating DrawIconEx step values.
/// Returns a Vec of RGBA frame data. If cursor is not animated, returns 1 frame.
unsafe fn probe_animation_frames(
    hicon: HICON,
    width: u32,
    height: u32,
) -> Result<Vec<Vec<u8>>> {
    ...
    for step in 0..MAX_ANIM_FRAMES {
        let rgba = match render_cursor_frame(hicon, width, height, step) {
            Ok(data) => data,
            Err(e) => {
                if step == 0 {
                    return Err(e); // First frame must succeed
                }
                // DrawIconEx can fail for steps beyond the animation length
    ...
    for y in 0..height as usize {
        for x in 0..width as usize {
            ...
            if and_val == 0 && xor_val == 0 {
                ...
                // AND=1, XOR=1: screen inversion area - render dark for visibility
                rgba[out_idx] = 0;
                rgba[out_idx + 1] = 0;
                rgba[out_idx + 2] = 0;
                rgba[out_idx + 3] = 200;
